...
│   ├── test_api_limits.py            # CLI/API 数量限制参数测试
│   ├── test_douyin_creator_limit.py  # 抖音 creator 模式数量限制测试
│   ├── test_db_sync.py               # 数据库结构同步脚本
│   ├── test_mongodb_integration.py   # MongoDB 集成测试(无服务时自动跳过)
│   └── ...                           # 其余平台 client/store/工具测试用例
├── tools
